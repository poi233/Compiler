package com.compiler.SyntaxAnalyzer;
			...
			try {
				if (input_cache.get(0).equals(deduce_str.get(deduce_str.size() - 1))) {
					...
					for (int i = 0; i < input_cache.size(); i++) {
						input = input + input_cache.get(i) + " ";
					}
					process = "";
					for (int i = deduce_str.size() - 1; i > -1; i--) {
						process = process + deduce_str.get(i) + " ";
					}
					if (input.equals("~ ") && process.equals("~ ")) {
						tbmodel_lex_result.addRow(new String[]{process, input, "accept"});
					} else {
						tbmodel_lex_result.addRow(new String[]{process, input, ""});
					}
...
//			}
			...
			if ((right = predictmap.get(leftandinput)) != null) {
				...
				tbmodel_lex_result.addRow(new String[]{process, input, deduce_str.get(deduce_str.size() - 1) + " -> " + right});
			...
			// 否则的话报错
				...
				tbmodel_lex_result.addRow(new String[]{process, input, "ERROR!  无法识别的字符" + input_cache.get(0) + "产生式" + leftandinput});
				input_cache.remove(0);
			}
		}
	}
